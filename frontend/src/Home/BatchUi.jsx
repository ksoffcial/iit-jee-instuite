...
const BatchUi = () => {
  ...
  const { enrolledCourses, enrolledCourseIds, isPaidUser, } = useSelector((state) => state.enroll)

  useEffect(() => {
    const enrollDetails = async () => {
      const respond = await dispatch(enrolmentDetails())
      console.log("batch details", respond)
    }
    enrollDetails()
  }, [dispatch])
